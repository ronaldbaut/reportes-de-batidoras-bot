...
async def manejar_respuesta(message):
    ...
    # ================== REPORTES DE BATIDORAS (agrupados por batidora) ==================
        ...
        if paso == 1:
            ...
            state["paso"] = 8
            await message.channel.send(f"8. ¿Están completos los dientes del piñón de la batidora {bat}? (envía foto)")
        elif paso == 8:
            state["paso"] = 9
        ...
        if paso == 1:
            ...
            if bat < 5:
                ...
                state["paso"] = 1
                await message.channel.send(f"\n**Batidora {bat + 1}**")
                await message.channel.send("1. ¿La temperatura del cabezote está por debajo de 50°?")
            else:
        ...
        if paso == 1:
            ...
            if bat < 5:
                ...
                state["paso"] = 1
                await message.channel.send(f"\n**Batidora {bat + 1}**")
                await message.channel.send("1. ¿Revisaste los dientes del piñón? (envía foto)")
            else:
